	...
	public class ExternalCombustionStatusEffect extends StatusEffect{
		...
		public void tick() {
			...
			if(isNthTick(10))nearbyPlayers.stream().filter(enemyCheck).forEach(dmgAction
					...
					.andThen(p -> getWorld().playEffect(((LivingEntity) p).getEyeLocation().subtract(Utils.NombreEntre(-0.1, 0.1), Utils.NombreEntre(-1.8, 0.1), Utils.NombreEntre(-0.1, 0.1)), Effect.SMOKE, 0))
