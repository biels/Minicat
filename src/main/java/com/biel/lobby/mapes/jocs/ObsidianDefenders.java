		...
		if (evt.getItem() != null){
			...
			if (stack.getType() == Material.DIAMOND_BLOCK){
				...
				for (Player p : obtenirEquip(plyr).getPlayers()){
					...
					ItemStack steveItem = new ItemStack(Material.SKELETON_SKULL, 1, (short)SkullType.PLAYER.ordinal());
			...
			if (stack.getType() == Material.NETHER_STAR){
				...
				evt.getItem().setType(Material.FIREWORK_STAR);
			}
			...
			if (stack.getType() == Material.BLAZE_POWDER){    				
				...
				for(Entity e : plyr.getNearbyEntities(25, 40, 25)){
					if (e instanceof Player){
						Player p = (Player)e;
						if (!obtenirEquip(p).equals(obtenirEquip(plyr))){
							if (pObj == null){
								pObj = p;
							}else{
