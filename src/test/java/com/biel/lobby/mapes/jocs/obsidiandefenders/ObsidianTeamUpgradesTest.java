...
public final class ObsidianTeamUpgradesTest {
    ...
    void purchaseProgression() {
        ...
        require(buy(upgrades, 0, 10, wallet, payments) == ObsidianTeamUpgrades.Purchase.BOUGHT, "launchers bought");
        ...
        for (int elapsed = 0; elapsed < 1200; elapsed++) {
            ...
            require(buy(upgrades, 0, 10 + elapsed, wallet, payments) == ObsidianTeamUpgrades.Purchase.LOADING, "rapid clicks blocked");
        }
        require(wallet[0] == 250 && payments[0] == 1, "loading never charges");
        require(Arrays.equals(ObsidianUpgradeController.lines(upgrades, 0, 10),
        ...
        wallet[0] = 99;
        require(buy(upgrades, 0, 1210, wallet, payments) == ObsidianTeamUpgrades.Purchase.FAILED, "100g required");
        require(upgrades.ready(0, 1210) && upgrades.level(0) == 1, "failure does not restart lock");
        wallet[0] = 250;
        require(buy(upgrades, 0, 1210, wallet, payments) == ObsidianTeamUpgrades.Purchase.BOUGHT, "archers at exact minute");
        require(wallet[0] == 150 && upgrades.has(0, ObsidianTeamUpgrades.Upgrade.ARCHERS), "archers apply immediately");
        require(!upgrades.has(0, ObsidianTeamUpgrades.Upgrade.ARMOR), "armor still locked");
        require(buy(upgrades, 0, 2409, wallet, payments) == ObsidianTeamUpgrades.Purchase.LOADING, "second full minute");
        require(buy(upgrades, 0, 2410, wallet, payments) == ObsidianTeamUpgrades.Purchase.BOUGHT, "armor bought for 150g");
        ...
        require(buy(upgrades, 0, 9999, wallet, payments) == ObsidianTeamUpgrades.Purchase.COMPLETE, "cannot buy level four");
        require(new ObsidianTeamUpgrades().level(0) == 0, "fresh match resets progression");
