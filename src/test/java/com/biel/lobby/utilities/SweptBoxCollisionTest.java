...
public final class SweptBoxCollisionTest {
    ...
    void collisionShapes() {
        ...
        SweptBoxCollision.Hit slabLanding = hit(new Vector(0.5, 2, 0.5), new Vector(0, -3, 0), slab);
        ...
        SweptBoxCollision.Hit wallHit = hit(new Vector(0, 1, 0.5), new Vector(2, 0, 0), insetWall);
        ...
        SweptBoxCollision.Hit diagonal = SweptBoxCollision.sweep(new Vector(0, 0, 0.5), HALF_EXTENTS,
        ...
        SweptBoxCollision.Hit recovery = hit(embedded, new Vector(), slab);
        if (!recovery.startedInside || SweptBoxCollision.overlaps(recovery.position, HALF_EXTENTS, slab)) {
        ...
        if (SweptBoxCollision.overlaps(new Vector(0.5, 0.65, 0.5), HALF_EXTENTS, slab)) {
            throw new AssertionError("touching is not overlap");
        }
        System.out.println("SweptBoxCollision checks passed");
    }

    private static SweptBoxCollision.Hit hit(Vector center, Vector movement, BoundingBox obstacle) {
        SweptBoxCollision.Hit result = SweptBoxCollision.sweep(center, HALF_EXTENTS, movement, List.of(obstacle));
    ...
    private static void noHit(Vector center, Vector movement, BoundingBox obstacle, String message) {
        if (SweptBoxCollision.sweep(center, HALF_EXTENTS, movement, List.of(obstacle)) != null) throw new AssertionError(message);
    }
